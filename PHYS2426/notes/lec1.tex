...
charge of an proton $e = 1.602\times 10^{-19}~{\rm C}$;
...
\i {\em Coulomb's law}:
The electric force on point charge 
$q_1$ due to point charge $q_2$, 
...
\ee
%
and $\hat r$ is a unit vector pointing from $q_2$ to $q_1$.
...
\i The electric force on point charge $Q$ due to a set of point charges
$q_1$, $q_2$, ..., $q_N$ is given by the vector sum of the 
forces 
%
\be
\vec F = \sum_{i=1}^N \frac{k_e Q q_i}{r_i^2}\,\hat r_i
\ee
%
where $r_i$ and $\hat r_i$ are the separation and unit vector
pointing from $q_i$ to $Q$.
