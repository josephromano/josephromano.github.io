...
### Earth model (radius of curvature in meters)
#     semimaj is equatorial radius
#     semimin is polar radius
earth_model = {'semimaj':6378137., 'semimin':6356752.314}
...
def Rlocal(latitude):
    ...
    RvH: Is this function the 'Normal' radius of curvature?
    '''
...
def dbasis_in_earthfixed(site):
    ...
    RvH: Should these tangent vectors not include the non-spherical nature of
    the Earth? Now it seems that the edy is not really tangential to the Earth's
    surface.
    '''
...
def detector_orient_vec(site):
    ...
    RvH: Again, should these tangent vectors not include the non-spherical
    nature of the Earth? Now it seems that the edy is not really tangential to
    the Earth's surface.
    '''
...
if __name__ == '__main__':
    ...
    freq = 100.             # Number of frequencies (not used yet)
    Nt = 100                # Number of times we re-calculate the signal response

    # Temporarily only do one detector ######## REMOVE THIS AGAIN!!! ###########
    ifo_dict = {'H1':ifo_dict['H1']}

    Ndet = len(ifo_dict)       # Number of detectors 
    ...
    t = np.linspace(0, sideday, Nt, endpoint=False)
    ...
    H = np.zeros((Ndet*Nt, 2*nPix), dtype=np.complex128)
    ...
    for kk in range(Ndet):
        ...
        for ii in range(nPix):
            ...
            print theta, phi, psi, u_t, v_t, r_t, freq
    ...
    #Sinv = np.zeros((Ndet*Nt, 2*nPix), dtype=np.complex128).T
