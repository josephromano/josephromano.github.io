...
elseif strcmpi(detector, 'HO') % Hobart
    ...
    baryinput.site.location(3) = -4311667.5;
...
elseif strcmpi(detector, 'VL') % VLA
    baryinput.site.location(1) = -1601192.;
...
baryinput.alpha = source.alpha;
baryinput.delta = source.delta;
baryinput.dInv = 0;
...
if ~isstruct(ephemE) || ~isstruct(ephemS)
    if ephemE == 0 || ephemS == 0
        disp('Error reading in one of the ephemeris files');
        emitdt = 0;
        emitte = 0;
        emitdd = 0;
        emitR = 0;
