function recover_skies_pixels_ifo_translate(sourcetype, detectortype, nPix, N, Nt)
%
% demo routine for recovering sky map using ifo data saved to a file
% using the translation-recover-inverse translation method
%
% Inputs:
...
%   'max_sourcetype_detectortype_pix_nPix_ifo_N_times_Nt_translate.jpg (and .mat)'
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all
...
% forward-translate simulated map
t_sim = translate_map([hpPix, hcPix], -rES, f);
h_sim = transpose(t_sim); % to make into a column vector
...
% inverse-translate ML recovered map
t_ML = translate_map([hp_ML; hc_ML], rES, f);
hp_ML = t_ML(1:end/2);
hc_ML = t_ML(end/2+1:end);

% save all data to .mat file
filename = ['max_' sourcetype '_' detectortype '_pix_' num2str(nPix) '_ifo_' num2str(N) '_times_' num2str(Nt) '_translate.mat'];
save(filename)
...
outfile = ['max_' sourcetype '_' detectortype '_pix_' num2str(nPix) '_ifo_' num2str(N) '_times_' num2str(Nt) '_translate.jpg'];
print('-djpeg', outfile);
