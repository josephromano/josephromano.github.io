function overlap_ifo_rotation(f, T, deltaT)
%
...
% times in earth's daily rotation
%
...
for ii=1:N
  ...
  % detector 2 rotates with earth
  phi = 2*pi*t(ii)/T_day;
  r2 = [RE*cos(phi); RE*sin(phi); 0];
  u2 = [-sin(phi); cos(phi); 0];
  v2 = [0; 0; 1];
  %u2 = [cos(phi); sin(phi); 0];
  %v2 = [-sin(phi); cos(phi); 0];
  ...
  if absDeltaX == 0 % this holds for pure rotation
    theta0 = 0;
    phi0 = 0;
  else
...
xlabel('time/1 day')
ylabel('overlap(t)')
outfile = ['overlap_ifo_rotation_f_' num2str(f) '_deltaT_' num2str(deltaT) '.eps'];
print('-depsc2', outfile);

figure
plot(diag(D))
ylabel('eigenvalues')
outfile = ['overlap_ifo_rotation_eigvalues_f_' num2str(f) '_deltaT_' num2str(deltaT) '.eps'];
print('-depsc2', outfile);
